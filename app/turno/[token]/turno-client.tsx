...
type Props = {
  funcionario: FuncionarioWithEmpresa
  initialTarefas: Tarefa[]
  initialExecucoes: Execucao[]
}

export default function TurnoClient({ funcionario, initialTarefas, initialExecucoes }: Props) {
  ...
  const tarefasDoTurno = initialTarefas.filter(
  ...
  async function handleCheck(tarefa: Tarefa) {
    if (isFeita(tarefa.id) || loadingId) return
